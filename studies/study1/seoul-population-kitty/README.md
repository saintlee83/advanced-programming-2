...
| 비교 리포트 | 세 열 표 | 왼쪽 요약 + 오른쪽 항목별 A/B 카드 |
| 지역 찾기 | 상단 검색 바 | 왼쪽 검색·정렬 + 오른쪽 순위표 |
| 파일 입력 | 세로 배치 | 두 파일을 가로로 배치 |

14가지 분석, 데이터 검증, 지역 교환, PNG·CSV·텍스트 내보내기를 지원한다.
...
| 비교 리포트 | 하루 흐름 유사도·차이가 가장 큰 시간·일평균 차이, 항목별 A/B 비교 카드, 관련 차트로 이동, 텍스트 리포트 저장 |
...
- 색상은 [Loading.io Hello Kitty 팔레트](https://loading.io/color/feature/HelloKitty/)의
  ...
  배경과 경계에는 옅은 변형을, 어두운 테마에는 읽기 쉬운 밝은 변형을 사용한다.
...
- 빠르게 페이지와 차트를 바꿔도 현재 선택 상태는 즉시 반영된다. 창 크기와 테마를 바꾸면 진행 중인 전환을 정리한다.
- 상단 오른쪽 **애니메이션 끄기**로 전환 효과를 끌 수 있다. 실행 전 `HOTPLACE_REDUCED_MOTION=1`로도 설정한다.

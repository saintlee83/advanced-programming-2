...
여기서 **Original**은 현재 저장소의 `seoul-population`을 뜻한다. 강의 최초 예제만을 가리키는 이름이 아니다. 두 버전 모두 `hotplace.py`에 `analysis1`부터 `analysis14`까지 있고, Kitty 앱은 그중 1~13만 사용한다.
...
`hotplace/hotplace.py`는 `analysis11`만 다르다. Original은 여성 비율 한 계열, Kitty는 여성·남성 두 계열을 돌려준다. `hotplace/dataset.py`는 기본 데이터 폴더를 찾는 `default_data_dir()`이 다르고 Kitty 쪽에 파일 종류 판별 `identify_csv()`가 추가되어 있다. `hotplace/__init__.py`는 설명 문구만 다르다. 집계와 검증 코드는 같다.
...
| 분석 선택 | `SegmentedWidget` + `Pivot`, 14개 차트 | `ComboBox` 두 개, 11개 차트(히트맵 2개 비활성화, 서울 속 위치 삭제) |
| 주요 지표 | 큰 숫자 카드 | A/B 두 칸짜리 비교표 한 장 |
| 지역 교환 | 버튼 있음 | 삭제 |
| 일반 A/B 차트·피라미드 | 좌우 `subplots(1, 2)` | 위아래 `subplots(2, 1)` |
| A/B 히트맵 | 위아래, 공통 색 범위 | GUI에서 비활성화(`--check --out`에서만 그림) |
| 값 판독 | 커서 옆 `HoverCallout` | 구현은 있으나 꺼 둠(`HOVER_ENABLED = False`). 차트 하단에는 읽는 법만 표시 |
| 성비 차트 | 여성 비율 한 선 | 여성(실선)·남성(점선) 두 선 |
...
겹쳐 보기·흐름 비교·시간대별 차이·연령 비중(Original은 서울 속 위치도)은 두 지역을 한 결과로 표현하므로 위 표의 일반 A/B 차트 배치와 구분해서 읽는다.
...
2026-09-18, Kitty를 PyQt5로 옮기고 애니메이션을 제거한 뒤, 화면을 ‘데이터 불러오기 → 차트 보기’로 줄이고 히트맵을 비활성화했다. 2026-09-19에는 파일 선택 창을 ‘항상 맨 앞’으로 바꾸고, 지역 교환과 서울 속 위치 차트를 삭제하고, 주요 지표를 비교표로 바꿨으며, 남녀 비율 차트에 남성 선을 추가하고 마우스 판독을 껐다. 두 선이 마주 보는 차트에서 정점 라벨이 겹치지 않도록 라벨을 다른 선의 반대쪽에 두게 고쳤다. Kitty 폴더의 환경에서 테스트 37개가 통과했다. 옮기기 전후의 화면을 같은 크기로 캡처해 비교했을 때 달라진 픽셀은 화면당 0.4% 미만(글자 가장자리)이었다. 아래 표는 옮기기 전의 기록이다.
